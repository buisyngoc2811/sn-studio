...
export const Marketplace: React.FC<MarketplaceProps> = ({ isLoggedIn, setRoute }) => {
  ...
  const [marketItems, setMarketItems] = useState<MarketplaceItem[]>(marketplaceData);

  useEffect(() => {
    const loadMarketItems = () => {
      const stored = localStorage.getItem('sn_market_db');
      if (stored) {
        setMarketItems(JSON.parse(stored));
      } else {
        localStorage.setItem('sn_market_db', JSON.stringify(marketplaceData));
        setMarketItems(marketplaceData);
      }
    };
    loadMarketItems();
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 space-y-10">
      ...
      {/* Product Grid with layout support */}
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 flex gap-4">
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-zinc-900 border border-zinc-800 group-hover:border-brand-500/30 group-hover:bg-brand-500/10 transition-all duration-300">
                    ...
                    <div>
                      ...
                      {/* Stats & Seller */}
                      <div className="mt-3 flex items-center gap-3 text-[10px] text-zinc-550">
